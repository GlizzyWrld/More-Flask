...
class UserViewsTestCase(TestCase):
    ...
    def test_login_required_for_following(self):
        """When logged out, are you disallowed from visiting a user's following page?"""
        with self.client as client:
            response = client.get(f"/users/{self.user.id}/following", follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assertIn(b"Access unauthorized.", response.data)

    def test_login_required_for_followers(self):
        """When logged out, are you disallowed from visiting a user's followers page?"""
        with self.client as client:
            response = client.get(f"/users/{self.user.id}/followers", follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assertIn(b"Access unauthorized.", response.data)

    def test_logged_in_user_can_see_following(self):
        """When logged in, can you see the following page for any user?"""
        with self.client as client:
            with client.session_transaction() as session:
                session["user_id"] = self.user.id

            response = client.get(f"/users/{self.user.id}/following")
            self.assertEqual(response.status_code, 200)
            self.assertIn(b"Following", response.data)

    def test_logged_in_user_can_see_followers(self):
        """When logged in, can you see the followers page for any user?"""
        with self.client as client:
            with client.session_transaction() as session:
                session["user_id"] = self.user.id

            response = client.get(f"/users/{self.user.id}/followers")
            self.assertEqual(response.status_code, 200)
